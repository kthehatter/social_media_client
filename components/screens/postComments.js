...
import { uploadCommentApiCall } from "../../controllers/screens/comments/uploadComment";
...
export default function PostComment(props) {
  ...
  const [showCreateCommentModel, setShowCreateCommentModel] = useState(false);
  ...
  const [commentImageCompressed, setCommentImageCompressed] = useState(null);
  ...
  const onDrop = (e) => {
    ...
    reader.onload = (loadEvt) => {
      ...
      if (type === "video/mp4") {
        setCommentVideo(loadEvt.target.result);
        setSelectedVideoUrl(URL.createObjectURL(files[0]));
      } else {
        setCommentImage(loadEvt.target.result);
      }
    };
  };
  const uploadComment = async () => {
    setIsUploading(true);
    await uploadCommentApiCall(
      props.postId,
      commentContent,
      commentVideo,
      commentImage
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          setShowCreateCommentModel(false);
          props.setCommentsList((prevState) => [
            { ...res.data.comment },
            ...prevState,
          ]);
          props.setCommentsNumber((prevState) => prevState + 1);
          console.log(props.commentsList);
          if (props.postOwner !==userInfo.username) {
            socket.emit("sendNotification", {
              reciever_id_finder: props.postId,
              notificationInfo: {
                type: "post comment",
      ...
      <div className="flex">
        ...
        <div className="flex-1">
          <button
            ...
            className={
              !isUploading && (data || commentContent !== "")
                ? `bg-blue-400 hover:bg-blue-500 mt-5 text-white font-bold py-2 px-4 rounded-full mr-4 float-right`
                : ` bg-gray-600 mt-5 text-white font-bold py-2 px-4 rounded-full mr-4 float-right`
            }
          >
            {!isUploading && <span>Reply</span>}
            {isUploading && (
