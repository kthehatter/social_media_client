...
import { recommendedPeopleToFollowApiCall } from "../../controllers/screens/user/follow";
import FollowButton from "./follow/followButton";
export default function RightSideBar() {
  const [peopleToFollow, setPeopleToFollow] = useState([]);
  const recommendedPeopleToFollow = async () => {
    try {
      await recommendedPeopleToFollowApiCall()
        .then((res) => {
          if (res.status === 200) {
            console.log("people to follow", res.data.peopleToFollow);
            setPeopleToFollow(res.data.peopleToFollow);
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  useEffect(() => {
    recommendedPeopleToFollow();
  }, []);
  return (
    <>
      <div className=" h-screen sticky top-0 border-l  border-gray-800 hidden lg:flex lg:flex-col  px-2 w-1/4 overflow-hidden lg:w-2/5 xl:w-2/6">
